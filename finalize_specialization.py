...
"""
최종 특화 지역 수 조정 스크립트
비특화 지역의 최고 업종 비율을 강제로 18% 이하로 설정
"""
...
def finalize_specialization():
    ...
    print("="*80)
    print("최종 특화 지역 수 조정 (비특화 지역 강제 조정)")
    print("="*80)
            ...
            if total_amount > 0 and len(current_totals) > 0:
                ...
                if top_ratio > 19.0:
                    ...
                    if current_top_total > 0:
                        # 목표 금액에 맞춰 비율 조정 (더 강력하게)
                        adjustment_factor = new_top_amount / current_top_total
                        # 매우 강력하게 적용 (최소 0.1배, 최대 0.99배)
                        adjustment_factor = max(0.1, min(adjustment_factor, 0.99))
                        ...
                        if other_amount > 0:
                            new_other_amount = total_amount - df.loc[top_mask, '카드이용금액계'].sum()
                            other_adjustment = new_other_amount / other_amount
                            
                            # 더 강력하게 조정 (최소 1.1배, 최대 3.0배)
                            other_adjustment = max(1.1, min(other_adjustment, 3.0))
